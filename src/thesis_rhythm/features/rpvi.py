...
def load_alignments(csv_paths: str | Path | list[str] | list[Path], use_phones39: bool = True) -> pd.DataFrame:
    ...
    # Sanity check: each utterance should belong to one speaker
...
def rpvi(durations: np.ndarray) -> float:
...
def extract_utterance_rpvi_metrics(df: pd.DataFrame, vowels: Iterable[str]) -> pd.DataFrame:
    vowels = set(vowels)
    rows = []
    ...
    for utt_id, g in df.groupby("utter_id", sort=False):
        ...
        rows.append({
            ...
            "rpvi_all": rpvi(all_durs),
            "rpvi_vowel": rpvi(vowel_durs),
            "rpvi_consonant": rpvi(consonant_durs),
        })

    out = pd.DataFrame(rows)
    if out.empty:
        raise RuntimeError("No utterance-level rPVI rows were produced.")
