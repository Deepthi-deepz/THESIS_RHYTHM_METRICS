...
def load_alignments(csv_paths: str | Path | list[str] | list[Path], use_phones39: bool = True) -> pd.DataFrame:
    """
    Load one or more alignment CSVs and concatenate them.
    """
...
def npvi(durations: np.ndarray) -> float:
    ...
    Normalized Pairwise Variability Index:
      nPVI = 100 * mean( |d[i+1] - d[i]| / ((d[i+1] + d[i]) / 2) )

    Returns NaN if fewer than 2 durations.
    """
...
def extract_utterance_npvi_metrics(df: pd.DataFrame, vowels: Iterable[str]) -> pd.DataFrame:
    """
    Compute utterance-level nPVI on:
      - full phone sequence
      - vowel subsequence
      - consonant subsequence
    """
    vowels = set(vowels)
    rows = []
    ...
    for utt_id, g in df.groupby("utter_id", sort=False):
        ...
        rows.append({
            ...
            "npvi_all": npvi(all_durs),
            "npvi_vowel": npvi(vowel_durs),
            "npvi_consonant": npvi(consonant_durs),
        })

    out = pd.DataFrame(rows)
    if out.empty:
        raise RuntimeError("No utterance-level nPVI rows were produced.")
