...
def extract_utterance_rhythm_bundle(df: pd.DataFrame, vowels: Iterable[str]) -> pd.DataFrame:
    vowels = set(vowels)
    rows = []
    ...
    if out.empty:
        raise RuntimeError("No utterance-level rhythm bundle rows were produced.")
